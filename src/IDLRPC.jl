...
function rpc_cleanup()
   ecode = ccall((:IDL_RPCCleanup, idlrpc), Cint, (Ptr{Nothing}, Cint), pclient.ptr, 0)
   if ecode != 1
...
function execute_converted(str::AbstractString)
   ...
   ecode = ccall((:IDL_RPCExecuteStr, idlrpc), Cint, (Ptr{Nothing},Ptr{UInt8}), pclient.ptr, str)
   if ecode != 1
      # since error get printed by IDL, we just reset error state
      ecode = ccall((:IDL_RPCExecuteStr, idlrpc), Cint, (Ptr{Nothing},Ptr{UInt8}), pclient.ptr,
      "message, /RESET")
      flush()
      return false
   end
...
function capture(flag::Bool)
   nlines = flag ? 5000 : 0
   ecode = ccall((:IDL_RPCOutputCapture, idlrpc), Cint, (Ptr{Nothing}, Cint), pclient.ptr, nlines)
   if ecode != 1
...
function get_output!(line_s::IDL_RPC_LINE_S)
   ecode = ccall((:IDL_RPCOutputGetStr, idlrpc), Cint, (Ptr{Nothing},Ref{IDL_RPC_LINE_S},Cint),
...
function put_var(arr::Array{T,N}, name::AbstractString) where {T,N}
   ...
   (Ptr{Nothing}, Ptr{UInt8}, Ptr{IDL_Variable}),
...
function put_var(x, name::AbstractString)
   ...
   vptr = get_vptr(name)
   ccall((:IDL_RPCStoreScalar, idlrpc), Nothing,
...
function put_var(str::AbstractString, name::AbstractString)
   ...
   ccall((:IDL_RPCStrStore, idlrpc), Nothing, (Ptr{IDL_String}, Ptr{Cchar}),
   idl_string, str)
   ecode = ccall((:IDL_RPCSetVariable, idlrpc), Cint,
   (Ptr{Nothing}, Ptr{UInt8}, Ptr{IDL_Variable}),
   pclient.ptr, name, vptr)
...
function get_vptr(name::AbstractString)
   ...
   ccall((:IDL_RPCGetMainVariable, idlrpc), Ptr{IDL_Variable}, (Ptr{Nothing},Ptr{UInt8}),
   pclient.ptr, name)
end

function get_var(name::AbstractString)
   vptr = ccall((:IDL_RPCGetMainVariable, idlrpc), Ptr{IDL_Variable}, (Ptr{Nothing},Ptr{UInt8}),
   pclient.ptr, name)
   ...
   vptr = ccall((:IDL_RPCDeltmp, idlrpc), Nothing, (Ptr{IDL_Variable},), vptr)
   var
end
